...
class SignalParser:
    ...
    async def parse(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._normalise_payload(data, message)
    ...
    @staticmethod
    def _normalise_payload(raw: Dict[str, Any], original_message: str) -> Dict[str, Any]:
        ...
        normalised_entry_keyword = entry_raw.strip().lower()
        entry_is_market_keyword = normalised_entry_keyword in {"market", "a mercado", "market order"}
        entry_value = "MARKET" if entry_is_market_keyword else entry_raw.replace(" ", "")

        max_entry_raw = _string(raw.get("max_entry"))
        max_entry_value = max_entry_raw.replace(" ", "") if max_entry_raw not in {"", "NA"} else ""

        message_lower = original_message.lower()
        entry_contains_hint = any(hint in message_lower for hint in _MARKET_HINTS)
        if entry_contains_hint and entry_value != "MARKET":
            if not max_entry_value:
                max_entry_value = entry_value
            entry_value = "MARKET"
        ...
        max_entry = max_entry_value or "NA"

        return {
            "symbol": symbol,
            "action": action,
            "entry": entry_value if entry_value else "MARKET",
            "max_entry": max_entry,
